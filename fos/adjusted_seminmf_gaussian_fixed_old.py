...
def _step(params, counts, mask, mean_func, sparsity_penalty, elastic_net_frac, num_coord_ascent_iters, data_variance):
    ...
    Args:
        ...
        mean_func: Function to compute mean
        sparsity_penalty: Penalty for sparsity in loadings
    ...
    Returns:
        tuple: (new_params, loss, param_changes)
    """
    import jax.numpy as jnp
    from jax import lax
    ...
    param_changes = {
        'loadings': jnp.abs(params.count_loadings - old_params.count_loadings).mean(),
        'factors': jnp.abs(params.factors - old_params.factors).mean(),
        'row_effects': jnp.abs(params.count_row_effects - old_params.count_row_effects).mean(),
        'col_effects': jnp.abs(params.count_col_effects - old_params.count_col_effects).mean()
    }
    
    return params, loss, param_changes


def fit_gaussian_seminmf(counts, initial_params, mask=None, mean_func="softplus", num_iters=100, sparsity_penalty=0.0, elastic_net_frac=0.0, num_coord_ascent_iters=10, tolerance=1e-3, data_variance=None):
    """Fit a semi-NMF model with Gaussian loss.
    ...
    # Create progress bar
    pbar = tqdm(range(num_iters), desc="Fitting model")
    
    # Main optimization loop
    for i in pbar:
        # Take one step
        params, loss, param_changes = _step(params, counts, mask, mean_func, sparsity_penalty, elastic_net_frac, num_coord_ascent_iters, data_variance)
        
        # Store loss
        losses.append(loss)
        
        # Compute held-out log likelihood if mask is provided
        if mask is not None:
            hll = compute_heldout_loglikelihood(params, counts, mask, mean_func, data_variance)
            heldout_loglikes.append(hll)
        
        # Update progress bar
        pbar.set_postfix({
            'loss': f'{loss:.3f}',
            'loadings': f'{param_changes["loadings"]:.2e}',
            'factors': f'{param_changes["factors"]:.2e}',
            'row_effects': f'{param_changes["row_effects"]:.2e}',
            'col_effects': f'{param_changes["col_effects"]:.2e}'
        })
        ...
        if i > 0:
            ...
            if relative_change < tolerance:
                ...
                print(f"Previous loss: {losses[-2]:.6f}")
